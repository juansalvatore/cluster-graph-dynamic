...
;(function(global, factory) {
  ...
  d3.chart('hierarchy', {
    initialize: function() {
      ...
      chart.d3.colorScale = chart.options.colors
        ? d3.scale.ordinal().range(chart.options.colors)
  ...
  */
          ...
          'exit:transition': function() {
            this.duration(chart.options.duration).remove()

            this.select('circle').attr('r', 0)

            this.select('text').style('fill-opacity', 0)
          },
        },
      })
      ...
      chart.layer('links', chart.layers.links, {
        dataBind: function(nodes) {
          return this.selectAll('.link').data(
            chart.d3.layout.links(nodes),
            function(d) {
              return d.target._id
            }
          )
        ...
        events: {
          ...
          'merge:transition': function() {
            this.duration(chart.options.duration).attr('d', chart.d3.diagonal)
          },
  ...
  d3.chart('cluster-tree').extend('cluster-tree.cartesian', {
    initialize: function() {
      ...
      chart.layers.nodes.on('merge:transition', function() {
        this.duration(chart.options.duration).attr('transform', function(d) {
      ...
      chart.off('change:margin').on('change:margin', function() {
        chart.options.width =
          chart.base.attr('width') -
          chart.options.margin.left -
          chart.options.margin.right
        chart.options.height =
          chart.base.attr('height') -
          chart.options.margin.top -
          chart.options.margin.bottom
        chart.layers.base.attr(
          'transform',
          'translate(' +
            chart.options.margin.left +
  ...
  d3.chart('cluster-tree').extend('cluster-tree.radial', {
    initialize: function() {
      ...
      chart.diameter(
        chart.options.diameter ||
          Math.min(chart.options.width, chart.options.height)
      )

      chart.d3.diagonal = d3.svg.diagonal.radial().projection(function(d) {
        return [d.y, d.x / 180 * Math.PI]
      })
      chart.d3.zoom.translate([
        chart.options.diameter / 2,
        chart.options.diameter / 2,
      ])
  ...
  d3.chart('hierarchy').extend('pack.flattened', {
    initialize: function() {
      ...
      chart.d3.zoom.translate([
        (chart.options.width - chart.options.diameter) / 2,
        (chart.options.height - chart.options.diameter) / 2,
      ])
      ...
      chart.layer('base', chart.layers.base, {
        dataBind: function(nodes) {
          return this.selectAll('.pack').data(
        ...
        events: {
          enter: function() {
            ...
            this.append('title').text(chart.options.bubble.title)
    ...
    transform: function(root) {
      ...
      return chart.d3.layout
        ...
        .padding(1.5)
        .nodes(
          chart.options.bubble.flatten
            ? chart.options.bubble.flatten(root)
            : root
        )
    },
  ...
  d3.chart('hierarchy').extend('pack.nested', {
    ...
    collapsible: function() {
      ...
      function collapse(d) {
        ...
        t.selectAll('.pack').attr('transform', function(d) {
          return 'translate(' + x(d.x) + ',' + y(d.y) + ')'
        })

        t.selectAll('circle').attr('r', function(d) {
          return k * d.r
        })

        t.selectAll('text').style('opacity', function(d) {
          return k * d.r > 20 ? 1 : 0
        })
  ...
  d3.chart('hierarchy').extend('partition.arc', {
    initialize: function() {
      ...
      chart.d3.zoom.translate([
        chart.options.width / 2,
        chart.options.height / 2,
      ])
      ...
      chart.layer('base', chart.layers.base, {
        ...
        events: {
          enter: function() {
            ...
            this.attr('d', chart.d3.arc).style('fill', function(d) {
              return chart.d3.colorScale(
                (d.children ? d : d.parent)[chart.options.name]
              )
            })
    ...
    collapsible: function() {
      ...
      function arcTween(d) {
        var xd = d3.interpolate(chart.d3.x.domain(), [d.x, d.x + d.dx]),
          ...
          yr = d3.interpolate(chart.d3.y.range(), [
            d.y ? 20 : 0,
            chart.options.diameter / 2,
          ])
  ...
  d3.chart('hierarchy').extend('partition.rectangle', {
    ...
    collapsible: function() {
      ...
      function collapse(d) {
        var kx =
            (d.y ? chart.options.width - 40 : chart.options.width) / (1 - d.y),
        ...
        t.selectAll('.partition').attr('transform', function(d) {
          return 'translate(' + x(d.y) + ',' + y(d.x) + ')'
        })
  ...
  d3.chart('hierarchy').extend('treemap', {
          ...
          enter: function() {
            ...
            this.append('rect')
              ...
              .attr('fill', function(d) {
                return d.parent
                  ? chart.d3.colorScale(d.parent[chart.options.name])
                  : null
              })
    ...
    collapsible: function() {
      ...
      function collapse(d) {
        ...
        t.selectAll('.cell').attr('transform', function(d) {
          return 'translate(' + x(d.x) + ',' + y(d.y) + ')'
        })
